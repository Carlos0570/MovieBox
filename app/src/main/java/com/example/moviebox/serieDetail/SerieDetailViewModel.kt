package com.example.moviebox.serieDetail
...
import com.example.moviebox.core.domain.SerieCreditsUseCase
import com.example.moviebox.core.domain.SerieDetailUseCase
import com.example.moviebox.core.domain.SerieImagesUseCase
import com.example.moviebox.core.domain.SerieProvidersUseCase
import com.example.moviebox.core.domain.SimilarSeriesUseCase
import dagger.hilt.android.lifecycle.HiltViewModel
import kotlinx.coroutines.flow.MutableStateFlow
import kotlinx.coroutines.launch
import javax.inject.Inject
import com.example.moviebox.core.util.awaitAll
import com.example.moviebox.core.domain.SerieTrailersUseCase
...
class SerieDetailViewModel @Inject constructor(
    ...
    private val serieImagesUseCase: SerieImagesUseCase
) : ViewModel() {
    ...
    val showTrailer = _showTrailer

    private val _serieImages = MutableStateFlow<MediaImage?> (null)
    val serieImages = _serieImages
    ...
    private suspend fun getSerieImages(serieId: Int) {
        when (val result = serieImagesUseCase(serieId)) {
            is Result.Error -> if (result.exception is IOException)
                state.value = ScreenState.FAILURE(result.errorMessage)

            is Result.Success -> _serieImages.value = result.data
        }
    }
}