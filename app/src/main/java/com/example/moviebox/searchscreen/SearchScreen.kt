...
import com.example.moviebox.home.SmallCard
...
private fun SearchResult(mediaItems: List<MediaItem>, title: String, onClick: (Int) -> Unit) {
    if (mediaItems.any { it.posterPath != null }) {
        Text(text = title)
        LazyRow {
            items(mediaItems) { mediaItem ->
                mediaItem.posterPath?.let {
                    SmallCard(imageURL = mediaItem.posterPath ?: mediaItem.backdropPath ?: it) {
...
private fun SearchPersonResult(persons: List<Person>, onClick: (Int) -> Unit) {
    if (persons.any { it.profilePath != null }) {
